...
final class HUDCoordinator {
    ...
    func applySettings() {
        guard let settings, FeatureFlag.hud.isEnabled else { stop(); return }
    ...
    private func present(_ reading: HUDReading) {
        // Done first, so the system overlay is chased away while ours is appearing rather
        // than after the user has already seen both.
        if settings?.huds.suppressSystemOverlay == true {
            SystemOSDSuppressor.suppress()
        }
